...
ENV_AUTO_SOURCE_CHAIN = "BS_AUTO_SOURCE_CHAIN"
...
def _fetch_from_source(
    ...
    *,
    ...
    raise ValueError(f"Unsupported BS source mode: {source}")
...
def _sharetrip_fetch_with_bdfare_fallback(
    ...
    LOG.warning("[BS] ShareTrip/BDFare returned no usable rows; attempting Gozayaan fallback")
...
def _bdfare_fetch_with_sharetrip_fallback(
    ...
    LOG.warning("[BS] BDFare returned no usable rows; attempting ShareTrip fallback")
    sharetrip_out = fetch_from_sharetrip(
        airline_code="BS",
        origin=origin,
        destination=destination,
    ...
    LOG.warning("[BS] BDFare/ShareTrip returned no usable rows; attempting Gozayaan fallback")
    gozayaan_out = fetch_from_gozayaan(
        airline_code="BS",
        origin=origin,
        destination=destination,
...
def fetch_flights(
    ...
    """
    if not source_enabled("bs"):
        return disabled_source_response("bs")
